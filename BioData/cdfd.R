...
home <- 'G:\\NC\\summer\\Mandar\\fwddeliverables\\bioData'
setwd(home)
source("functions.R")
...
data <- read.csv('bio_transposed.csv', row.names=1)

#raw_rainfall_data <- read.csv('Rainfall Data.csv', header = TRUE)

# Enter the months for the rainfall season. 
#season <- 1:1
#seasonal_rainfall_data <- data[, season]
...
years <- matrix(1:71, nrow=num_years, ncol=1, dimnames=list(rownames(raw_data)))

build_c50 <- predict_c50 <- result_c50 <- list()

disc_org_rainfall_data <-matrix(0, nrow=num_years, ncol=1)
disc_org_rainfall_data[, 1] <- getDiscretizedResponse(raw_data, col_names)
...
avg_acc_per_run <- 0
num_folds <- 8
accuracies <- matrix(nrow = length(cv_run), ncol = num_folds)
n <- 0
...
#i = 1
  ...
  #j = 2
    ...
    test_data <- read.table(paste(cv_dirs[j], test_data_file, sep='/'), header=TRUE,  sep='\t')
    cg_file_path <- file.path(paste(cv_dirs[j], 'Causal_Graphs', sep='/'))
    
    # Read Causal Graph from training data
    pc_graph_file <- list.files(path=cg_file_path, pattern='^BioData_*.*.xml')
    ...
    print(paste(cg_file_path, cg_filename, sep="/"))
    ...
    rainfall_idx <- which(pc_graph@nodes=='Response')
    
    disc_rainfall_data <- discVariable(as.matrix(train_data[, 'Response']))   # Discretize the normalized rainfall 
    #print("***********************************")
    #print(disc_rainfall_data)
    colnames(disc_rainfall_data) <- 'Response'
    true_class <- disc_org_rainfall_data[test_years]
    #print(true_class)
    ...
    train_edge_features$Response <- as.factor(disc_rainfall_data)
    ...
    curr_acc <- calcAccuracy(true_class, model[[2]])
    accuracies[i,j] <- curr_acc
    avg_acc_per_cv <- avg_acc_per_cv + curr_acc
    print(curr_acc)
    #print(true_class)
  ...
  print((avg_acc_per_cv / n))
  avg_acc_per_run <- avg_acc_per_run + (avg_acc_per_cv / n) #add avg accuracy for this run
  avg_num_features <- avg_num_features / num_folds
}

overall_acc <- avg_acc_per_run / length(cv_run)
print(accuracies)
write.csv(accuracies, "G:\\NC\\summer\\Mandar\\fwddeliverables\\bioData\\results.csv", append = TRUE)
