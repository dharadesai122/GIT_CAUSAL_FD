  ...
  if (method=="local") {
    ...
    if (y.pos %in% pa1) {
      ...
      if (length(pa2)==0) {
        ...
        pcr.model <- pcr(Response~., data=data[,c(x.pos,pa1,y.pos)])
        ...
        for (i2 in 1:length(pa2)) {
          ...
          if (!tmpColl) {
            ...
            if (y.pos %in% pa2.t) {
              ...
              pcr.model <- pcr(Response~., data=data[,c(x.pos,pa1,pa2[i2],y.pos)])
              max.var.comp <-  which.max(explvar(pcr.model))
              beta.hat[ii] <- data.frame(pcr.model$coefficients)[idx.name, max.var.comp]
              var.hat[ii] <- explvar(pcr.model)[max.var.comp]
              # Compute p-value of the causal effect
            ...
            for (j in 1:n.comb) {
              ...
              if (!tmpColl) {
                ...
                if (y.pos %in% pa2.t) {
                  ...
                  pcr.model <- pcr(Response~., data=data[,c(x.pos,pa1,pa.tmp[,j],y.pos)])
                  max.var.comp <-  which.max(explvar(pcr.model))
                  beta.hat[ii] <- data.frame(pcr.model$coefficients)[idx.name, max.var.comp]
                  var.hat[ii] <- explvar(pcr.model)[max.var.comp]
                  # Compute p-value of the causal effect
