...
preprocessData <- function(i, orgClimData, numYears, response_cols){
  
  #print (dim(orgClimData))
  idx <- numYears[-i]
  sampleClimData <- as.matrix(orgClimData[-i, -which(names(orgClimData) %in% response_cols)])
  testClimData <- as.matrix(orgClimData[i, -which(names(orgClimData) %in% response_cols)])
  
  responseData <- as.matrix(orgClimData[-i, response_cols])
...
#   ============================================================
#   Comment out the following lines to detrend the rainfall data
#   ============================================================
...
# 
#   detrend_response <- detrendData(i, idx, responseData)
#   responseData <- detrend_response[[1]]
  ...
  sumResponseData <- matrix(0, nrow=nrow(responseData), ncol=1)  
  colnames(sumResponseData) <- "Response"
  
  for(idx in 1:ncol(responseData)){  # Aggregate the detrended values of rainfall season into one column
    sumResponseData <- sumResponseData + responseData[, idx]
  }

  normResponseData <- normalizeData(sumResponseData, 'TRAIN')    # Normalized the aggregated rainfall values

  return(list(sampleClimData, testClimData, normResponseData))
}

discVariable <- function(responseData){
  
  discVector <- matrix(0, nrow=nrow(responseData), ncol=1)

  # Find the HIGH anomalous years for Var1_Month[i]
  sortedVector <- sort(responseData, decreasing=TRUE,index.return=TRUE)
...
getDiscretizedResponse <- function(orgClimData, response_cols){
  orgResponseData <- as.matrix(orgClimData[, response_cols])
  detrendOrgResponseData <- matrix(0, nrow=nrow(orgResponseData), ncol=ncol(orgResponseData))
  sumResponseData <- matrix(0, nrow=nrow(orgResponseData), ncol=1)
  
  # ===========================================================
  # Detrend and aggregate
  # Should be used only for climate data   
  #   detrendOrgRainfallData <- apply(orgRainfallSeasonData, 2, detrend)
  # ============================================================

  for(i in 1:ncol(detrendOrgResponseData)){
    sumResponseData <- sumResponseData + orgResponseData[, i]
  }
  
  # Normalize and discretize rainfall
  normResponseData <- normalizeData(sumResponseData, 'TRAIN')
  discResponseData <- discVariable(normResponseData)
  print(discResponseData)
  return(discResponseData)
}
...
buildModel <- function(topKFeatures, oneYearDataKFeatures, features=NULL, response, featureSelection=FALSE){
  ...
  if(featureSelection){
    ...
    trainData <- topKFeatures[, c(features, 'Response')]
...
computePValue <- function(idx_name, beta, data, pred_idx, response_idx, flag){
    ...
    for(i in 1:rand_perm){
      ...
      if(flag==1)
        reg_model <- pcr(Response~., data=data[, c(pred_idx, response_idx)])
      else
        reg_model <- plsr(Response~., data=data[, c(pred_idx, response_idx)], method="oscorespls")
...
kfold_cv <- function(data, k){
  ...
  #season <- 3:5
  col_names <- paste("q_RIBFLV", sep="")

  for(j in 1:10){
    
    dir_name <- 'G:\\NC\\summer\\Mandar\\fwddeliverables\\bioData'
    ...
    for(i in 1:length(subsample_list)){
      ...
      colnames(process_rainfall_data) <- 'Response'
      ...
      write.table(norm_train_data, paste(data_dir_name, '/BioData_CV_', i, '.txt', sep=""), row.names=FALSE, sep='\t')
